  ...
  setup() {
    ...
    async function updateChart(selectedValue) {
      const resultSet = await cubejsApi.load({
        ...
        measures: ["loan.count"],
        dimensions: ["borrower.firstname", "borrower.lastname"],
      });
      const dataLabel = resultSet.loadResponses[0].data.filter(
        (obj, index, f) =>
          f.findIndex(
            (item) => item[`borrower.firstname`] === obj[`borrower.firstname`]
          ) === index
      );

      if (chartInstance) {
        // Update the chart data and labels
        chartInstance.data.labels = resultSet.loadResponses[0].data.map(
          (da) => moment(da[`loan.applicationdate`]).format("MMMM-DD-YYYY")
        );
        chartInstance.data.datasets = dataLabel.map((measure) => ({
          label: getLegendName(measure[`borrower.firstname`]),
          data: resultSet
            .chartPivot()
            .map(
              (m) =>
                m[
                  `${measure[`borrower.firstname`]},${
                    measure[`borrower.firstname`]
                  },loan.count`
                ]
            ),
        ...
        const measures = resultSet.loadResponses[0].data.map(
          (da) => da[`loan.applicationdate`]
        );
